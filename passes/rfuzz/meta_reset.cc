...
public:
	MetaResetWorker(RTLIL::Module *_module, bool _opt_verbose, string _opt_signame, bool _opt_fixedname, bool is_top) {
		opt_verbose = _opt_verbose;
		opt_signame = _opt_signame;
		opt_fixedname = _opt_fixedname;
...
struct MetaResetPass : public Pass {
	...
	void execute(std::vector<std::string> args, RTLIL::Design *design) override
	{
		...
		for (auto &module: topo_modules.sorted) {
			MetaResetWorker(module, opt_verbose, opt_signame, opt_fixedname, module==top_module);
		}
	}
} MetaResetPass;
